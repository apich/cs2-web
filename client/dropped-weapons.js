...
export class DroppedWeapons{
 ...
 sync(snapshot){const receivedAt=performance.now();this.drops=(snapshot.droppedWeapons||[]).map(d=>({...d,receivedAt}));}
 update(player,camera,active,binding='E'){
  const now=performance.now(),nearby=this.drops.map(d=>({d,distance:Math.hypot(d.x-camera.position.x,d.y-camera.position.y,d.z-camera.position.z)})).filter(o=>o.distance<28).sort((a,b)=>a.distance-b.distance).slice(0,12),keep=new Set(nearby.map(o=>o.d.id));
  ...
  for(const {d,distance}of nearby){
   let item=this.items.get(d.id);if(!item&&getSkin(d.skinId)){
    ...
    if(!source&&distance<12&&this.pending.size<2&&!this.pending.has(d.skinId)&&(this.retry.get(d.skinId)||0)<=now){
      this.retry.set(d.skinId,now+30000);const task=loadSkin(d.skinId).catch(()=>{}).finally(()=>this.pending.delete(d.skinId));this.pending.set(d.skinId,task);
    }
    if(source&&this.items.size<12){
      ...
      this.scene.add(group);retainSkin(d.skinId);item={group,skinId:d.skinId,heightOffset:height*.5-DROP_PHYSICS.radius+.006};this.items.set(d.id,item);
    }
   }
   ...
   if(item){const position=extrapolateDrop(d,(now-d.receivedAt)/1000);item.group.position.set(position.x,position.y+item.heightOffset,position.z);item.group.rotation.set(0,d.yaw||0,Math.PI/2);}
